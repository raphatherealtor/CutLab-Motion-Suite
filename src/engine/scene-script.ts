...
export function deriveSceneScript(
    ...
    for (const seg of speakerSegments) {
      ...
      if (!alreadyCovered) {
        regions.push({
          ...
          start: fromSeconds(seg.startSecs, 30000),
          end: fromSeconds(seg.endSecs, 30000),
  ...
  for (const marker of sequence.markers) {
    ...
    regions.push({
      ...
      start: marker.time,
      end: fromSeconds(markerSecs + 0.5, 30000),
  ...
  if (seqClips.length > 0) {
    ...
    if (seqDuration > 2) {
      ...
      regions.push({
        id: generateId('scene-region'),
        kind: 'intro',
        start: fromSeconds(0, 30000),
        end: fromSeconds(introDur, 30000),
        label: 'Intro',
        energyLevel: 'low',
        cameraEnergy: 'restrained',
      ...
      regions.push({
        id: generateId('scene-region'),
        kind: 'outro',
        start: fromSeconds(seqDuration - outroDur, 30000),
        end: fromSeconds(seqDuration, 30000),
        label: 'Outro',
        energyLevel: 'low',
        cameraEnergy: 'static',
        compositionDensity: 'sparse',
